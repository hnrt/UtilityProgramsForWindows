...
static WCHAR s_Buf[SLOT_COUNT][BUF_LENGTH];
static ULONG s_Next;


static PWCHAR Allocate()
{
    return s_Buf[InterlockedIncrement(&s_Next) % SLOT_COUNT];
}


static PCWSTR Format(PWCHAR pBuf, PCWSTR pszFormat, UINT offset)
{
    _snwprintf_s(pBuf, BUF_LENGTH, _TRUNCATE, pszFormat, offset);
    return pBuf;
}


static PCWSTR Format(PCWSTR pszFormat, UINT offset)
{
    return Format(Allocate(), pszFormat, offset);
}
...
#define C(x) case x: return L#x
    ...
    default:
        if (m_value < WM_USER)
        {
            return Format(L"WM_%04X", m_value);
        }
        else if (m_value < WM_APP)
        {
            return Format(L"WM_USER_%04X", m_value - WM_USER);
        }
        else if (m_value < 0xC000)
        {
            return Format(L"WM_APP_%04X", m_value - WM_APP);
        }
        else if (m_value < 0x10000)
        {
            PWCHAR pBuf = Allocate();
            if (GetClipboardFormatNameW(m_value, pBuf, BUF_LENGTH))
            {
                return pBuf;
            }
            return Format(pBuf, L"?%04X?", m_value);
        }
        else
        {
            return Format(L"?%08X?", m_value);
        }
    }
}