...
StringBuffer& Base64Encoder::Append(const void* ptr, size_t len)
{
...
StringBuffer& Base64Encoder::End()
{
    if (m_Remaining == 1)
    {
        if (m_len + 5 > m_cap)
...
bool Base64Decoder::Parse(PCWSTR psz)
{
    ...
    DBGPUT(L"DataLen=%zu", this->Len);

    PCWSTR pCur = psz;
    unsigned char* pDst = this->Ptr;
    unsigned char* pEnd = this->Ptr + this->Len;
    while (pDst + 3 <= pEnd)
    {
        ...
        *pDst++ = static_cast<unsigned char>((b0 << 2) | (b1 >> 4));
        *pDst++ = static_cast<unsigned char>(((b1 & 0x0F) << 4) | (b2 >> 2));
        *pDst++ = static_cast<unsigned char>(((b2 & 0x03) << 6) | b3);
        pCur += 4;
    }
