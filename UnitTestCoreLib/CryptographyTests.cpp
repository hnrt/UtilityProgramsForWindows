	...
	public:
		...
		TEST_METHOD(Test01)
		{
			Debug::Put(L"CryptographyTest01: Started.");
			{
				...
				Array<DWORD> bsList = hAlg.BlockSizeList;
				Debug::Put(L"CryptographyTest01: BlockSizeList=[%zu]{%s}", bsList.Length, JoinBy(bsList, L","));
				Assert::AreEqual(1UL, bsList.Length);
				Assert::AreEqual(16UL, bsList[0]);
				Array<DWORD> kList = hAlg.KeyLengths;
				Debug::Put(L"CryptographyTest01: KeyLengths=[%zu]{%s}", kList.Length, JoinBy(kList, L","));
				Assert::AreEqual(3UL, kList.Length);
		...
		TEST_METHOD(Test_AES_128_CBC_Encrypt)
		{
			...
			try
			{
				MultibyteString d(CP_UTF8, L"澤岻奈々子(タクシナナコ)");
				MultibyteString keyHint("MAX/NANA");
				...
				MultibyteString ivHint("20231204");
				...
				Base64Encoder e;
				e.Append(encrypted, encrypted.Length);
				e.End();
		...
		TEST_METHOD(Test_AES_128_CBC_Decrypt)
		{
			...
			try
			{
				...
				ByteString decrypted = hKey.Decrypt(d, d.Length, iv, iv.Len);
				String sz(CP_UTF8, decrypted, decrypted.Length);
		...
		TEST_METHOD(Test_AES_128_ECB_Encrypt)
		{
			...
			try
			{
				MultibyteString d(CP_UTF8, L"日本語123");
				MultibyteString keyHint("BENI");
		...
		TEST_METHOD(Test_AES_128_ECB_Decrypt)
		{
			...
			try
			{
				...
				ByteString decrypted = hKey.Decrypt(d, d.Length);
				String sz(CP_UTF8, decrypted, decrypted.Length);
		...
		TEST_METHOD(Test_AES_128_CFB)
		{
			...
			try
			{
				MultibyteString text(CP_UTF8, L"大谷翔平17");
				...
				MultibyteString ivHint("20231208");
				MD5Hash ivHash(ivHint, ivHint.Len); // 128 bits (16 bytes)
				Buffer<BYTE> iv(16);
				...
				memcpy_s(iv, iv.Len, ivHash.Value, iv.Len);
				ByteString decrypted = hKey.Decrypt(encrypted, encrypted.Length, iv, iv.Len);
				Debug::Put(L"Test_AES_128_CFB: decrypted=%s", decrypted.ToHex());
				Assert::IsTrue(String(CP_UTF8, reinterpret_cast<PCHAR>(decrypted.Ptr), decrypted.Length) == String(CP_UTF8, text.Ptr, text.Len));
		...
		TEST_METHOD(Test_AES_128_CCM)
		{
			...
			try
			{
				MultibyteString text(CP_UTF8, L"大谷翔平17二刀流");
				...
				Array<DWORD> tagLengths = hAlg.AuthTagLengths;
				Debug::Put(L"Test_AES_128_CCM: AuthTagLengths[%zu]=%s", tagLengths.Length, JoinBy(tagLengths, L","));
				...
				ByteString decrypted = hKey.Decrypt(encrypted, encrypted.Length, infoD, NULL, 0);
		...
		TEST_METHOD(Test_AES_128_CCM_CHAIN_CALLS)
		{
			...
			try
			{
				MultibyteString text(CP_UTF8, L"二刀流大谷翔平ドジャースに10年総額7億ドルで移籍、2024年シーズンは50/50を達成しワールドシリーズ制覇。ナショナルリーグMVPに選出され大活躍");
				...
				Array<DWORD> tagLengths = hAlg.AuthTagLengths;
				Debug::Put(L"Test_AES_128_CCM_CHAIN_CALLS: AuthTagLengths[%zu]={%s}", tagLengths.Length, StringUtils::JoinBy(tagLengths, L","));
				MultibyteString nonceHint("20231208");
				MD5Hash nonceHash(nonceHint, nonceHint.Len); // 128 bits (16 bytes)
				Buffer<BYTE> ivE(16);
				Buffer<BYTE> ivD(16);
				...
				Assert::IsTrue(MultibyteString(decrypted, decrypted.Len) == text);
		...
		TEST_METHOD(Test_AES_128_GCM)
		{
			...
			try
			{
				...
				Array<DWORD> tagLengths = hAlg.AuthTagLengths;
				Debug::Put(L"Test_AES_128_GCM: AuthTagLengths[%zu]=%s", tagLengths.Length, JoinBy(tagLengths, L","));
				MultibyteString nonceHint("20231208");
				MD5Hash nonceHash(nonceHint, nonceHint.Len); // 128 bits (16 bytes)
				BCryptAuthenticatedCipherModeInfo infoE;
				BCryptAuthenticatedCipherModeInfo infoD;
				...
				infoD
					...
					.SetTag(infoE.pbTag, infoE.cbTag);
				ByteString decrypted = hKey.Decrypt(encrypted, encrypted.Length, infoD, NULL, 0);
				Debug::Put(L"Test_AES_128_GCM: decrypted[%zu]=%s", decrypted.Length, decrypted.ToHex());
				Assert::IsTrue(String(CP_UTF8, reinterpret_cast<PCHAR>(decrypted.Ptr), decrypted.Length) == String(CP_UTF8, text.Ptr, text.Len));
		...
		TEST_METHOD(Test_AES_128_GCM_CHAIN_CALLS)
		{
			...
			try
			{
				MultibyteString text(CP_UTF8, L"二刀流大谷翔平ドジャースに10年総額7億ドルで移籍");
				...
				Array<DWORD> tagLengths = hAlg.AuthTagLengths;
				Debug::Put(L"Test_AES_128_GCM_CHAIN_CALLS: AuthTagLengths[%zu]=%s", tagLengths.Length, JoinBy(tagLengths, L","));
				MultibyteString nonceHint("20231208");
				MD5Hash nonceHash(nonceHint, nonceHint.Len); // 128 bits (16 bytes)
				Buffer<BYTE> ivE(16);
				Buffer<BYTE> ivD(16);
				...
				Debug::Put(L"Test_AES_128_GCM_CHAIN_CALLS: encrypted1[%zu]=%s", encrypted1.Length, encrypted1.ToHex());
				...
				Debug::Put(L"Test_AES_128_GCM_CHAIN_CALLS: encrypted2[%zu]=%s", encrypted2.Length, encrypted2.ToHex());
				Debug::Put(L"Test_AES_128_GCM_CHAIN_CALLS: tag=%s", ByteString(infoE.pbTag, infoE.cbTag).ToHex());
				Debug::Put(L"Test_AES_128_GCM_CHAIN_CALLS: iv=%s", ByteString(ivE, ivE.Len).ToHex());
				Debug::Put(L"Test_AES_128_GCM_CHAIN_CALLS: flags=%lx", infoE.dwFlags);
				...
				infoE
					.ResetFlags(BCRYPT_AUTH_MODE_CHAIN_CALLS_FLAG);
				ByteString encrypted3 = hKey.Encrypt(const_cast<PSTR>(text.Ptr) + 16 * 2, text.Len - 16LL * 2LL, infoE, ivE, ivE.Len);
				Debug::Put(L"Test_AES_128_GCM_CHAIN_CALLS: encrypted3[%zu]=%s", encrypted3.Length, encrypted3.ToHex());
				Debug::Put(L"Test_AES_128_GCM_CHAIN_CALLS: tag=%s", ByteString(infoE.pbTag, infoE.cbTag).ToHex());
				Debug::Put(L"Test_AES_128_GCM_CHAIN_CALLS: iv=%s", ByteString(ivE, ivE.Len).ToHex());
				Debug::Put(L"Test_AES_128_GCM_CHAIN_CALLS: flags=%lx", infoE.dwFlags);
				...
				Debug::Put(L"Test_AES_128_GCM_CHAIN_CALLS: encrypted[%zu]=%s", encrypted.Length, encrypted.ToHex());
				// DECRYPTION #1
				infoD
					.SetNonce(nonceHash.Value, AES_GCM_NONCE_LENGTH)
					.SetTag(infoE.pbTag, infoE.cbTag)
				...
				Debug::Put(L"Test_AES_128_GCM_CHAIN_CALLS: decrypted1[%zu]=%s", decrypted1.Length, decrypted1.ToHex());
				...
				Debug::Put(L"Test_AES_128_GCM_CHAIN_CALLS: decrypted2[%zu]=%s", decrypted2.Length, decrypted2.ToHex());
				Debug::Put(L"Test_AES_128_GCM_CHAIN_CALLS: iv=%s", ByteString(ivD, ivD.Len).ToHex());
				// DECRYPTION #3
				infoD
					.ResetFlags(BCRYPT_AUTH_MODE_CHAIN_CALLS_FLAG);
				ByteString decrypted3 = hKey.Decrypt((PUCHAR)encrypted + 16 * 2, encrypted.Length - 16LL * 2LL, infoD, ivD, ivD.Len);
				Debug::Put(L"Test_AES_128_GCM_CHAIN_CALLS: decrypted3[%zu]=%s", decrypted3.Length, decrypted3.ToHex());
				Debug::Put(L"Test_AES_128_GCM_CHAIN_CALLS: iv=%s", ByteString(ivD, ivD.Len).ToHex());
				ByteString decrypted = decrypted1;
				decrypted += decrypted2;
				decrypted += decrypted3;
